...
assert.match(
  ...
  "radar strategy signal source should read the global strategy event pool",
);
assert.match(
  appShellSource,
...
assert.doesNotMatch(
  appShellSource,
  /return currentUser\.id\s*\?\s*`\/api\/strategy\/inbox/,
  "radar strategy hits should not use the account inbox as the primary strategy source",
);
assert.match(
  appShellSource,
